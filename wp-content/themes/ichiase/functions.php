...
/*-----------------------------------------------------------------------------------*/
define( 'MTS_THEME_NAME', 'socialme' );
...
function mts_after_setup_theme() {
    ...
    load_theme_textdomain( 'socialme', get_template_directory().'/lang' );

    add_theme_support( 'post-thumbnails' );
    set_post_thumbnail_size( 530, 250, true );
    add_image_size( 'socialme-featured', 530, 250, true ); //featured
    add_image_size( 'socialme-related', 220, 125, true ); //related
    add_image_size( 'socialme-sliderthumb', 128, 107, true ); //widget
    add_image_size( 'socialme-widgetthumb', 65, 50, true ); //slider thumbnail
    add_image_size( 'socialme-widgetfull', 328, 215, true ); //sidebar full width
    add_image_size( 'socialme-slider', 728, 342, true ); //slider

    add_action( 'init', 'socialme_wp_review_thumb_size', 11 );
    function socialme_wp_review_thumb_size() {
        add_image_size( 'wp_review_large', 328, 215, true ); 
        add_image_size( 'wp_review_small', 65, 50, true );
    }

    register_nav_menus( array(
      'primary-menu' => __( 'Primary', 'socialme' ),
      'footer-menu' => __( 'Footer', 'socialme' ),
      'mobile' => __( 'Mobile', 'socialme' )
    ) );
...
function mts_get_icons() {
    $icons = array(
        __( 'Web Application Icons', 'socialme' ) => array(
        ...
        __( 'File Type Icons', 'socialme' ) => array(
        ...
        __( 'Spinner Icons', 'socialme' ) => array(
            'circle-o-notch', 'cog', 'refresh', 'spinner'
        ),
        __( 'Form Control Icons', 'socialme' ) => array(
        ...
        __( 'Directional Icons', 'socialme' ) => array(
        ...
        __( 'Video Player Icons', 'socialme' ) => array(
        ...
        __( 'Medical Icons', 'socialme' ) => array(
...
function mts_get_featured_image($post_ID) {  
    ...
    if ($post_thumbnail_id) {  
        $post_thumbnail_img = wp_get_attachment_image_src($post_thumbnail_id, 'socialme-widgetfull');  
...
function mts_columns_head($defaults) {
    if (get_post_type() == 'post') {
        $defaults['featured_image'] = __('Featured Image', 'socialme' );
    }
...
function mts_register_sidebars() {
    ...
    register_sidebar( array(
        'name' => __('Left Sidebar', 'socialme' ),
        'description'   => __( 'Appears on all pages', 'socialme' ),
        'id' => 'left-sidebar',
    ...
    // Default sidebar
    register_sidebar( array(
        'name' => __('Right Sidebar', 'socialme'),
        'description'   => __( 'Appears on all pages', 'socialme' ),
        'id' => 'sidebar',
        'before_widget' => '<div id="%1$s" class="widget %2$s">',
        'after_widget' => '</div>',
    ...
    if ( !empty( $mts_options['mts_first_footer'] )) {
        ...
        register_sidebars( $mts_options['mts_first_footer_num'], array(
            'name' => __( 'Footer Column %d', 'socialme' ),
            'description'   => __( 'Appears in footer area.', 'socialme' ),
            'id' => 'footer-first',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
    ...
    if ( mts_is_wc_active() ) {
        ...
        register_sidebar( array(
            'name' => __('WC Left Sidebar', 'socialme' ),
            'description'   => __( 'Appears on Shop main page, product archive pages and on single product pages.', 'socialme' ),
            'id' => 'left-wc-sidebar',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
        ...
        register_sidebar( array(
            'name' => __('WC Shop Right Sidebar', 'socialme' ),
            'description'   => __( 'Appears on Shop main page and product archive pages.', 'socialme' ),
            'id' => 'shop-sidebar',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
        ...
        register_sidebar( array(
            'name' => __('WC Single Product Right Sidebar', 'socialme' ),
            'description'   => __( 'Appears on single product pages.', 'socialme' ),
            'id' => 'product-sidebar',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'after_widget' => '</div>',
            'before_title' => '<h3 class="widget-title">',
 ...
 * @return string
 */
function mts_custom_sidebar( $sidebar = 'sidebar' ) {
    $mts_options = get_option( MTS_THEME_NAME );

    if ( 'left-sidebar' === $sidebar ) {
        if ( is_home() && !empty( $mts_options['mts_sidebar_for_home_2'] )) $sidebar = $mts_options['mts_sidebar_for_home_2'];  
        if ( is_single() && !empty( $mts_options['mts_sidebar_for_post_2'] )) $sidebar = $mts_options['mts_sidebar_for_post_2'];
        if ( is_page() && !empty( $mts_options['mts_sidebar_for_page_2'] )) $sidebar = $mts_options['mts_sidebar_for_page_2'];
        
        // Archives.
        if ( is_archive() && !empty( $mts_options['mts_sidebar_for_archive_2'] )) $sidebar = $mts_options['mts_sidebar_for_archive_2'];
        if ( is_category() && !empty( $mts_options['mts_sidebar_for_category_2'] )) $sidebar = $mts_options['mts_sidebar_for_category_2'];
        if ( is_tag() && !empty( $mts_options['mts_sidebar_for_tag_2'] )) $sidebar = $mts_options['mts_sidebar_for_tag_2'];
        if ( is_date() && !empty( $mts_options['mts_sidebar_for_date_2'] )) $sidebar = $mts_options['mts_sidebar_for_date_2'];
        if ( is_author() && !empty( $mts_options['mts_sidebar_for_author_2'] )) $sidebar = $mts_options['mts_sidebar_for_author_2'];
        
        // Other.
        if ( is_search() && !empty( $mts_options['mts_sidebar_for_search_2'] )) $sidebar = $mts_options['mts_sidebar_for_search_2'];
        if ( is_404() && !empty( $mts_options['mts_sidebar_for_notfound_2'] )) $sidebar = $mts_options['mts_sidebar_for_notfound_2'];
        
        // Woocommerce.
        if ( mts_is_wc_active() ) {
            if ( is_shop() || is_product_category() || is_product() ) {
                $sidebar = 'left-wc-sidebar';
                if ( !empty( $mts_options['mts_sidebar_for_wc'] )) {
                    $sidebar = $mts_options['mts_sidebar_for_wc'];
                }
            }
        }
    } else {
...
include_once( "functions/widget-author-box.php" );
include_once( "functions/widget-author-box-featured.php" );
include_once( "functions/widget-users-total.php" );
include_once( "functions/widget-post-new.php" );
include_once( "functions/widget-login-form.php" );
...
function mts_add_scripts() {
    ...
    wp_localize_script(
        ...
        array(
            ...
            'like' => ( empty( $mts_options['mts_points_feature'] ) ? false : true ),
        )
    );
...
function mts_load_footer_scripts() {  
    ...
    if( ! empty( $mts_options['mts_pagenavigation_type'] ) && $mts_options['mts_pagenavigation_type'] >= 2 && !is_singular() ) {
        ...
        wp_localize_script(
            ...
            array(
                ...
                'autoLoad' => $autoload,
                'i18n_loadmore' => __( 'Load More', 'socialme' ),
                'i18n_loading' => __('Loading...', 'socialme' ),
                'i18n_nomore' => __( 'No more posts.', 'socialme' )
             )
        );
    }
    ...
    if ( defined( 'WP_REVIEW_URI' ) && wp_script_is( 'wp_review-js', 'registered' ) ) {
        if ( ( ! empty( $mts_options['mts_pagenavigation_type'] ) && $mts_options['mts_pagenavigation_type'] >= 2 && !is_singular() ) || ( !empty( $mts_options['mts_homepage_tabs'] ) && is_home() ) ) {
            wp_enqueue_script( 'wp_review-js' );
            if ( wp_script_is( 'jquery-knob', 'registered' ) ) wp_enqueue_script( 'jquery-knob' );
            if ( file_exists( WP_REVIEW_DIR . 'rating-types/circle-output.js' ) ) wp_enqueue_script( 'wp-review-circle-output', trailingslashit( WP_REVIEW_URI ) . 'rating-types/circle-output.js', array( 'jquery' ) );
        }
    }
}  
...
function mts_enqueue_css() {
    ...
    wp_enqueue_style( 'socialme-stylesheet', get_stylesheet_uri() );

    if ( wp_style_is( 'wp_review-style', 'registered' ) ) {
        if ( ( ! empty( $mts_options['mts_pagenavigation_type'] ) && $mts_options['mts_pagenavigation_type'] >= 2 && !is_singular() ) || ( !empty( $mts_options['mts_homepage_tabs'] ) && is_home() ) ) {
    ...
    $mts_header_without_logo = '';
    $mts_header_without_nav = '';
    $mts_header_without_login = '';
    $mts_header_without_search = '';
    $mts_article_category = '';
    $mts_layout = '';

    if ( is_page() || is_single() ) {
        $mts_sidebar_location = get_post_meta( get_the_ID(), '_mts_sidebar_location', true );
    }
    if ( $mts_sidebar_location != 'left' && ( $mts_options['mts_layout'] == 'cslayout' || $mts_sidebar_location == 'right' || $mts_options['mts_layout'] == 'csslayout' )) {
        $mts_sclayout = '.article { float: left } .sidebar.c-4-12.sidebar-left, .woocommerce .sidebar.c-4-12 { float: right }';
    }
    if ( empty( $mts_options['mts_header_section2'] ) ) {
        ...
        if ( !empty( $mts_options['mts_show_primary_nav'] ) || !empty( $mts_options['mts_header_search']) ){
            $mts_header_without_logo = '#primary-navigation { position: relative; float: left} 
            .searchbox { position: relative; float: right; }';
        }
    }
    if ( empty( $mts_options['mts_header_search'] ) ) {
        $mts_header_without_search = '#header { float: right; } 
        #primary-navigation { float: left; position: relative } ';
    }
    if ( empty( $mts_options['mts_show_primary_nav'] ) ) {
        $mts_header_without_nav = '#header { float: left; }';
    }
    if ( empty( $mts_options['mts_login_register'] ) ) {
        $mts_header_without_login = '#primary-navigation { bottom: 0!important; } header .searchbox { display: block!important; }';  
    }
    if ( empty( $mts_options['mts_home_meta_info_enable']['category'] ) ) {
        $mts_article_category = '.post-footer { padding-top: 10px; padding-bottom: 10px; }';
    }
    if ( isset( $mts_options['mts_social_button_position'] ) && $mts_options['mts_social_button_position'] == 'floating' ) {
        $mts_shareit_left = '.shareit { top: 282px; left: 0; width: 48px; position: fixed; padding: 0; border:none; border-right: 0;}
        .share-item {margin: 2px;}';
    }
    if ( ! empty( $mts_options['mts_author_comment'] ) ) {
        $mts_author = '.bypostauthor .fn:after { content: "'.__( 'Author', 'socialme' ).'"; padding: 1px 10px; color: #FFF; border-radius: 3px; font-size: 12px; margin: 0 4px; }';
    }

    if ( isset($mts_options['mts_layout']) && ($mts_options['mts_layout'] == 'cslayout' || $mts_options['mts_layout'] == 'sclayout' )) {
        $mts_layout = '.sidebar-left-article { width: 100%; max-width: 1170px; } .article { width: 65.81%;
    ...
    $custom_css = "
        ...
        #site-header .header-wrap, .navigation ul ul li, .mobile-menu-active .navigation.mobile-menu-wrapper, .mts-author-box-top { background: {$mts_options['mts_header_background']}; }

        #site-footer { background: {$mts_options['mts_footer_background']}; }

        #primary-navigation .navigation a, .navigation ul ul { border-color: {$mts_options['mts_header_background']}; }
        ...
        .featured-stories-tabs .tabs li.active a, .post-footer .thecategory a:hover, .tags a:hover, .post-footer .vote a:hover, .post-footer .vote.active a, .tagcloud a:hover, .woocommerce nav.woocommerce-pagination ul li a:hover, .woocommerce-page nav.woocommerce-pagination ul li a:hover, .woocommerce #content nav.woocommerce-pagination ul li a:hover, .woocommerce-page #content nav.woocommerce-pagination ul li a:hover, .pagination .nav-previous a, .pagination .nav-next a, .widget.mts-post-new, .mts-login-form #wp-submit {
            border: 1px solid {$mts_options['mts_second_color_scheme']};
        }
        .latestPost:hover, .article-footer .thecategory a:hover, .article-footer .readMore a:hover {
            border: 1px solid rgba(". mts_convert_hex_to_rgb($mts_options['mts_second_color_scheme']) . ", 0.3);
        }
        #thumb .owl-item.active.current img, blockquote, .latestPost .quotes {
            border-bottom: 4px solid {$mts_options['mts_second_color_scheme']};
        }
        {$mts_sclayout}
        {$mts_shareit_left}
        {$mts_author}
                    ...
                    <div id="comment-<?php comment_ID(); ?>">
                        <div class="comment-author vcard">
                            ...
                            <span class="comment-meta">
                                <?php edit_comment_link( __( '( Edit )', 'socialme' ), '  ', '' ) ?>
                            </span>
                        </div>
                        <?php if ( $comment->comment_approved == '0' ) : ?>
                            <em><?php _e( 'Your comment is awaiting moderation.', 'socialme' ) ?></em>
                            <br />
                        <?php endif; ?>
                    </div>
                <?php
...
if ( ! function_exists( 'mts_excerpt' ) ) {
    ...
    function mts_excerpt( $limit = 40 ) {
      return esc_html( mts_truncate( strip_tags(get_the_excerpt()), $limit, 'words' ) );
    }
}
    ...
    function mts_readmore() {
        ...
        <div class="readMore">
            <a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>">
                <?php _e( '<i class="fa fa-angle-up"></i>', 'socialme' ); ?>
            </a>
        </div>
        <?php 
    }
...
function ajax_mts_search() {
    ...
    if ( !empty( $query ) && $search_query->have_posts() ) : 
            ...
            ?><li>
                <a href="<?php echo esc_url( get_the_permalink() ); ?>">
                    <?php if ( has_post_thumbnail() ) { ?>
                        <?php the_post_thumbnail( 'socialme-widgetthumb', array( 'title' => '' ) ); ?>
                    <?php } else { ?>
        ...
        endwhile;
        echo '</ul>';
        echo '<div class="ajax-search-meta"><span class="results-count">'.$search_count.' '.__( 'Results', 'socialme' ).'</span><a href="'.esc_url( get_search_link( $query ) ).'" class="results-link">'.__('Show all results.', 'socialme' ).'</a></div>';
    else:
        echo '<div class="no-results">'.__( 'No results found.', 'socialme' ).'</div>';
    endif;
    wp_reset_postdata();
...
if ( mts_is_wc_active() ) {
    ...
    function mts_header_add_to_cart_fragment( $fragments ) {
        ...
        <a class="cart-contents" href="<?php echo esc_url( $woocommerce->cart->get_cart_url() ); ?>" title="<?php _e( 'View your shopping cart', 'socialme' ); ?>"><?php echo sprintf( _n( '%d item', '%d items', $woocommerce->cart->cart_contents_count, 'socialme' ), $woocommerce->cart->cart_contents_count );?> - <?php echo $woocommerce->cart->get_cart_total(); ?></a>
...
function mts_custom_gravatar( $avatar_defaults ) {
    ...
    $avatar_defaults[$mts_avatar] = __( 'Custom Gravatar ( /images/gravatar.png )', 'socialme' );
...
function mts_megamenu_thumbnails( $thumbnail_html, $post_id ) {
    ...
    if(has_post_thumbnail($post_id)):
        $thumbnail_html .= get_the_post_thumbnail($post_id, 'socialme-widgetfull', array('title' => ''));
    else:
        $thumbnail_html .= '<img src="'.get_template_directory_uri().'/images/nothumb-socialme-widgetfull.png" alt="'.__('No Preview', 'socialme').'"  class="wp-post-image" />';
    endif;
    $thumbnail_html .= '</a>';
...
function mts_admin_bar_link() {
    ...
    if( current_user_can( 'edit_theme_options' ) ) {
        $wp_admin_bar->add_menu( array(
            ...
            'title' => __( 'Theme Options', 'socialme' ),
...
/* Homepage Tabs
/*-----------------------------------------------------------------------------------*/
add_action('wp_ajax_mts_home_tabs_content',  'ajax_mts_home_tabs_content');
add_action('wp_ajax_nopriv_mts_home_tabs_content', 'ajax_mts_home_tabs_content');
function ajax_mts_home_tabs_content(){

    $mts_options = get_option(MTS_THEME_NAME);
    $tab = $_POST['tab'];
    $page = empty($_POST['page']) ? 1 : $_POST['page'];

    if( $tab == '#popular-tab-content'){

        $days = isset( $mts_options['mts_popular_posts_limit'] ) ? $mts_options['mts_popular_posts_limit'] : 365;
        $posts_order = isset( $mts_options['mts_popular_posts_order'] ) ? $mts_options['mts_popular_posts_order'] : 'views';
        $posts_per_page = isset( $mts_options['mts_popular_posts_num'] ) ? $mts_options['mts_popular_posts_num'] : 6;
        //Date
        $popular_days = array();
        if ( $days ) {
            $popular_days = array(
        ...
        $args = array('ignore_sticky_posts' => 1, 'posts_per_page' => $posts_per_page, 'post_status' => 'publish', 'orderby' => 'meta_value_num', 'meta_key' => '_mts_view_count', 'order' => 'desc', 'date_query' => $popular_days, 'paged' => $page);

        if( $posts_order == 'comment_count' ) {
            $args['orderby'] = 'comment_count';
        } else if( $posts_order == 'points' ) {
            $args['meta_key'] = '_mts_likes';
        }
        ...
        endwhile; 
            ...
            echo '<p>'.__('No Trending Posts Found!','socialme').'</p>';
        endif; 
        
        mts_home_pagination($tab, $page, $popular_query->max_num_pages);

        wp_reset_postdata();

    } elseif ( $tab == '#latest-tab-content') {

        $posts_per_page = isset( $mts_options['mts_latest_posts_num'] ) ? $mts_options['mts_latest_posts_num'] : 6;

        $args = array('ignore_sticky_posts' => 1, 'posts_per_page' => $posts_per_page, 'post_status' => 'publish', 'paged' => $page);
        $lat_query = new WP_Query($args);
        if ( $lat_query->have_posts() ) : while ( $lat_query->have_posts() ) : $lat_query->the_post(); ?>
            <article class="latestPost excerpt">
               <?php mts_archive_post(); ?>
            </article>
        <?php endwhile; endif;

        mts_home_pagination($tab, $page, $lat_query->max_num_pages);

        wp_reset_postdata();

    } elseif ( $tab == '#random-tab-content') {

        $posts_per_page = isset( $mts_options['mts_random_posts_number'] ) ? $mts_options['mts_random_posts_number'] : 6;

        $args = array('ignore_sticky_posts' => 1, 'posts_per_page' => $posts_per_page, 'post_status' => 'publish', 'paged' => $page, 'orderby' => 'rand');
        ...
        mts_home_pagination($tab, $page, $random_query->max_num_pages);
...
// Add profile fields
function mts_add_user_profile_fields($profile_fields) {

    // Add new fields
    $profile_fields['mts_city'] = __( 'City', 'socialme' );
    $profile_fields['mts_twitter'] = __( 'Twitter Username', 'socialme' );

    return $profile_fields;
}
add_filter('user_contactmethods', 'mts_add_user_profile_fields');
...
function mts_like_dislike($post_id = 0) {
    ...
    ?>
    <div class="vote <?php echo $like_class; ?>" data-postid="<?php echo $post_id; ?>">
        <a href="#" title="<?php _e( 'Upvote this post', 'socialme' ); ?>"><i class="fa fa-angle-up"></i></a>
    </div>
    <div class="mts-points"><span class="like-count"><?php echo (int) get_post_meta( $post_id, '_mts_likes', true ); ?></span> <span class="points-text"><?php _e('Points','socialme'); ?></span></div>
    <?php
}

if( !empty( $mts_options['mts_points_feature'] )) {
...
// check if current IP has liked post already
// returns false (didn't rate) or 1 (liked)
