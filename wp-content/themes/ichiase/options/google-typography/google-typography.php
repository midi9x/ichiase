...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title					= __('Adding A Collection', 'socialme' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'socialme' ).'</p>';
		$adding_content			 .= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'socialme' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'socialme' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'socialme' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'socialme' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'socialme' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'socialme' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'socialme' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'socialme' ).'</li><li><b>'.__('Font Variant', 'socialme' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'socialme' ).'</li><li><b>'.__('Font Size', 'socialme' ).'</b> - '.__('The size you would like this font to be.', 'socialme' ).'</li><li><b>'.__('Font Color', 'socialme' ).'</b> - '.__('The color you\'d like to use for this font.', 'socialme' ).'</li><li><b>'.__('CSS Selectors', 'socialme' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'socialme' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'socialme' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'socialme' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'socialme' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'socialme' ).':</b> '.__('#selector', 'socialme' ).'</li><li><b>'.__('Classes:', 'socialme' ).':</b> '.__('.selector', 'socialme' ).'</li><li><b>'.__('HTML Tags', 'socialme' ).':</b> '.__('span', 'socialme' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'socialme' ).':</b> '.__('#selector span.date', 'socialme' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'socialme' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title								= __('Theme Typography', 'socialme' );
		$loading							= __('Loading Your Collections', 'socialme' );
		$add_new        			= __('Add New Collection', 'socialme' );
		$reset        				= __('Reset Collections', 'socialme' );
		$preview_text   			= __('Type in some text to preview...', 'socialme' );
		$collection_title 			= __('New Collection', 'socialme' );
        //$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'socialme' );
		//$font_family_title  	= __('Font family...', 'socialme' );
        $std_font_family_title  	= __('Standard Fonts', 'socialme' );
        $google_font_family_title  	= __('Google Fonts', 'socialme' );
		$font_variant_title 	= __('Variant...', 'socialme' );
		$font_size_title 			= __('Size...', 'socialme' );
        $backup_font_title          = __('Fallback:', 'socialme' );
		$css_selectors_title	= __('CSS Selectors:', 'socialme' );
        $additional_css_title	= __('Additional CSS:', 'socialme' );
		$delete_button_text   = __('Delete', 'socialme' );
		$save_button_text			= __('Save', 'socialme' );
		
		$welcome_title 				= __('Theme Typography', 'socialme' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'socialme' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'socialme' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'socialme' );
		$step_1_desc					= __('Choose from any of the 600+ Google Fonts.', 'socialme' );
		$step_2_title 				= __('2. Customize It', 'socialme' );
		$step_2_desc					= __('Pick a size, variant, color and more.', 'socialme' );
		$step_3_title 				= __('3. Attach It', 'socialme' );
		$step_3_desc					= __('Attach your font to any CSS selector(s).', 'socialme' );
		$year									= date("Y");
        $themename = MTS_THEME_NAME;
        
        $character_sets_title = __('Character sets', 'socialme' );
        $character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'socialme' );
        ...
        $character_sets = array(
            'latin' => __('Latin', 'socialme' ),
            'latin-ext' => __('Latin Extended', 'socialme' ),
            'cyrillic' => __('Cyrillic', 'socialme' ),
            'cyrillic-ext' => __('Cyrillic Extended', 'socialme' ),
            'greek' => __('Greek', 'socialme' ),
            'greek-ext' => __('Greek Extended', 'socialme' ),
            'vietnamese' => __('Vietnamese', 'socialme' ),
            'khmer' => __('Khmer', 'socialme' ),
            'devanagari' => __('Devanagari', 'socialme' ),
        );
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
	            'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'socialme' ),
	            'delete_confirm' => __("Are you sure you want to delete this collection?", 'socialme' )
	        ));
