...
function __construct() {
	...
	$widget_ops = array(
		...
		'description' => __('A widget for 300 x 250 ad (Single banner)', 'socialme' )
	);
	...
	// Create the widget
	parent::__construct( 'mts_ad_300_widget', sprintf( __('%s300x250 Ad', 'socialme' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ), $widget_ops, $control_ops );
...
function widget( $args, $instance ) {
	...
	// Display Ad
	if ( $link )
		echo '<a href="' . esc_url( $link ) . '"><img src="' . esc_url( $ad ) . '" alt="" /></a>';
		
	elseif ( $ad )
	 	echo '<img src="' . esc_url( $ad ) . '" alt="" />';
...
function form( $instance ) {
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'ad' ); ?>"><?php _e('Ad image url:', 'socialme' ) ?></label>
	...
	<p>
		<label for="<?php echo $this->get_field_id( 'link' ); ?>"><?php _e('Ad link url:', 'socialme' ) ?></label>
