...
class single_category_posts_widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			sprintf( __('%sCategory Posts', 'socialme' ), MTS_THEME_WHITE_LABEL ? '' : 'MTS ' ),
			array( 'description' => __( 'Display the most recent posts from a single category', 'socialme' ) )
		);
	}
 	...
 	public function form( $instance ) {
		...
		$title = isset( $instance[ 'title' ] ) ? $instance[ 'title' ] : __( 'Featured Category', 'socialme' );
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'socialme' ); ?></label>
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'cat' ); ?>"><?php _e( 'Category:', 'socialme' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'qty' ); ?>"><?php _e( 'Number of Posts to show', 'socialme' ); ?></label>
		...
		<p>
	       <label for="<?php echo $this->get_field_id( 'title_length' ); ?>"><?php _e( 'Title Length:', 'socialme' ); ?>
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_thumb1"); ?>">
				...
				<?php _e( 'Show Thumbnails', 'socialme' ); ?>
			</label>
		</p>
		
		<p>
			<label for="<?php echo $this->get_field_id('box_layout'); ?>"><?php _e('Posts layout:', 'socialme' ); ?></label>
			<select id="<?php echo $this->get_field_id('box_layout'); ?>" name="<?php echo $this->get_field_name('box_layout'); ?>">
				<option value="horizontal-small" <?php selected($box_layout, 'horizontal-small', true); ?>><?php _e('Horizontal', 'socialme' ); ?></option>
				<option value="vertical-small" <?php selected($box_layout, 'vertical-small', true); ?>><?php _e('Vertical', 'socialme' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id("date"); ?>">
				...
				<?php _e( 'Show post date', 'socialme' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id("comment_num"); ?>">
				...
				<?php _e( 'Show number of comments', 'socialme' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id("show_excerpt"); ?>">
				...
				<?php _e( 'Show excerpt', 'socialme' ); ?>
			</label>
		</p>
		
		<p>
	       <label for="<?php echo $this->get_field_id( 'excerpt_length' ); ?>"><?php _e( 'Excerpt Length:', 'socialme' ); ?>
            ...
            if ( $show_thumb1 == 1 ) : ?>
				<div class="post-img">
					<a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>">
						<?php if(!empty($icon_class)) echo '<div class="post-format-icons"><i class="fa fa-'.$icon_class.'"></i></div>';
						if ( has_post_thumbnail() ) { ?>
							<?php the_post_thumbnail( 'socialme-' . $thumbnail, array( 'title' => '' ) ); ?>
						<?php } else { ?>
							<img class="wp-post-image" src="<?php echo get_template_directory_uri() . '/images/nothumb-socialme-' . $thumbnail . '.png'; ?>" alt="<?php echo esc_attr( get_the_title() ); ?>"/>
						<?php } ?>
					</a>
				</div>
			<?php endif; ?>
